...
- 期7 custom-only 配置收敛已完成；期8（v1.6.0）已删除 dormant provider 与旧兼容入口，并完成图片管线模块化。
...
- Provider 层只保留 `BaseVisionClient` + `CustomClient`；继续修改时必须保持连接配置与能力 profile 分离。
- 不扩展期5的 clipboard/latest、grounding 或视频高级能力，除非用户明确要求。
- 图片关键路径位于 `src/media/image-source.ts`、`image-transform.ts`、`image-crop.ts` 与 `prepare-image.ts`；改动必须配相关测试。
...
- `npm run test:smoke` 构建后启动真实 stdio MCP 进程，验证版本与 `tools/list`，不调用模型。
...
- Provider 主干只保留 custom；未来若恢复内置 provider，必须以文档与 live probe 兼容性矩阵为前置证据。
