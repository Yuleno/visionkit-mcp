...
- URL 归一化通用化重构（联网调研国内主流厂商后落地）：废除 hostname 白名单（`detectProtocol`/`detectStrictness`/`looksAnthropic` 全删）。协议优先级：显式 `VISIONKIT_PROTOCOL` > base_url 路径信号 > 默认 `openai`。路径信号 = hostname 为 `api.anthropic.com` 或路径精确含 `/anthropic` 段（百炼 `/apps/anthropic`、小米 `/anthropic`、智谱 `/api/anthropic` 等），精确段匹配不误伤 `/anthropic-bridge` 子串——因此 `.mcp.json` 配 `https://dashscope.aliyuncs.com/apps/anthropic` 无需显式设 protocol 即可自动走 Anthropic（已真实验收）。`VISIONKIT_ANTHROPIC_STRICTNESS` 默认 `vendor-loose`。归一化收敛为统一函数 `resolveEndpoint(rawBaseUrl, protocol)`，一条铁律：**代码只补协议资源路径（OpenAI `/chat/completions`、Anthropic `/v1/messages`），绝不补版本前缀**——版本前缀（`/v1`、`/v4`、`/compatible-mode/v1` 等）一律视为 base_url 的一部分原样保留。实测覆盖小米 MiMo、Kimi、DeepSeek、智谱（3 端点）、百炼（2 端点）、火山方舟、百度千帆、阶跃、自建 `evo4.local` 共 ~20 个真实端点，零厂商特判。
